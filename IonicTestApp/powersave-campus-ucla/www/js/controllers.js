...
.controller('HomeCtrl', function($scope, $ionicLoading, $state, $ionicModal, $ionicPlatform, Home, Highlights) {
	...
	otherResult.then( function( val ) {
        ...
        $scope.size = {
            "height": (winHeight - 207) + "px",
            "width": (0.679 * (winHeight - 207)) + "px",
            "margin": "0 auto"
        };
		/*$scope.imageClass = {
...
// New controllers
.controller('PowerCalcCtrl', function( $scope, $state, $ionicPlatform ) {
	var option = -1;
	$scope.title = $state.current.data.title;
    $scope.data = {
        lights1: 0,
	...
	$scope.watts;
    var watts = 0;
    var blank = angular.copy($scope.data);
    ...
    var update = function() {
        switch( option ) {
			case -1:
				option = 0;
				$scope.watts = (watts * 0.001).toFixed(3) + " kWh/month";
				break;
			...
			case 1:
				$scope.watts = "$" + (watts * 0.001 * 0.152).toFixed(2) + " saved";
		}
    }
    update();
    
	$scope.recalc = function() {
		watts = ($scope.data.lights1 * 1.6 * 30 * 60) + ($scope.data.lights2 * 46 * 48) + ($scope.data.lights3 * 50 * 48) + ($scope.data.lights4 * 4 * 48) + ($scope.data.heat1 * 30000) + ($scope.data.heat2 * 429) + ($scope.data.unplug * 12500) + ($scope.data.water1 * 1440) + ($scope.data.water2 * 8100) + ($scope.data.water3 * 740);
		update();
	}
    ...
    $scope.reset = function() {
        $scope.data = angular.copy(blank);
        option = 0;
        $scope.recalc();
    }
    
	$scope.footerAdj = function() {
		return $ionicPlatform.is("iOS");
	}
});